...
// SỬA LỖI: Thêm regenerateSingleImagePrompt vào import
...
// ... (Giữ nguyên các hàm splitTextIntoChaptersLocally, splitTextIntoNChapters) ...
...
const ScriptGenerator: React.FC<ScriptGeneratorProps> = ({ apiKey }) => {
  ...
  const handleGenerateScript = async () => {
    const scriptText = storyChapters.map((c, i) => `--- BẮT ĐẦU CHƯƠNG ${i + 1} ---\n${c.text}\n--- KẾT THÚC CHƯƠNG ${i + 1} ---`).join('\n\n').trim();
    const currentMode = scriptText ? 'script' : 'idea';
    const currentInput = currentMode === 'script' ? scriptText : ideaInput;

    if (!currentInput.trim()) { setError("Vui lòng nhập ý tưởng/kịch bản."); return; }
    if (selectedStylePrompts.length === 0) { setError("Vui lòng chọn phong cách."); return; }

    setMode(currentMode); setIsLoading(true); setError(null); setScript(null);

    try {
      const combinedStylePrompt = selectedStylePrompts.join(', ');
      const result = await generateScript(
          currentInput, currentMode, combinedStylePrompt, characterReferences, characterDefinition, 
          aspectRatio, generateImage, generateMotion, includeMusic, dialogueLanguage, apiKey, characterSource
      );
      setScript(result);
    } catch (err) { setError(err instanceof Error ? err.message : 'Lỗi không xác định.'); } 
  ...
  // --- HÀM MỚI: Xử lý Tạo lại Prompt ---
  const handleRegeneratePrompt = async (sceneIndex: number) => {
    if (!script || !script.scenes[sceneIndex]) return;

    // 1. Cập nhật state: Đang tạo lại
    setScript(prev => {
        if (!prev) return null;
        const newScenes = [...prev.scenes];
    ...
    try {
        ...
        // 2. Xác định Nguồn Chân Lý (Giống hệt logic tạo kịch bản)
        ...
        // 4. Cập nhật Prompt mới vào bảng
        setScript(prev => {
            if (!prev) return null;
            const newScenes = [...prev.scenes];
