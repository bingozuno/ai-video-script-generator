...
// =========================================================================
// === SỬA LỖI: CẬP NHẬT PROMPT HỆ THỐNG (PHIÊN BẢN 8.0 - INJECT DỮ LIỆU) ===
// =========================================================================
...
**TỐI HẬU THƯ VỀ NHẤT QUÁN NHÂN VẬT (TUÂN THỦ TUYỆT ĐỐI - PHIÊN BẢN 8.0)**

Nhiệm vụ quan trọng nhất của bạn là đảm bảo nhân vật nhất quán.

**1. NGUỒN DỮ LIỆU NHÂN VẬT (Nguồn Chân Lý):**
* DƯỚI ĐÂY LÀ "NGUỒN CHÂN LÝ VỀ NHÂN VẬT" MÀ BẠN PHẢI SỬ DỤNG. ĐÂY LÀ NGUỒN DUY NHẤT.
---
{{CHARACTER_DATA_GOES_HERE}}
---

**2. QUY TRÌNH BẮT BUỘC KHI VIẾT PROMPT ẢNH ("DÁN VÀ SỬA"):**
Khi bạn viết \`Prompt Tạo Ảnh\` cho một cảnh có nhân vật (ví dụ: 'Haruka Sato'):

* **BƯỚC 1: TÌM VÀ DÁN (COPY & PASTE)**
    * TÌM mô tả của 'Haruka Sato' trong \`NGUỒN CHÂN LÝ VỀ NHÂN VẬT\` (đã cung cấp ở trên).
    * BẮT BUỘC **SAO CHÉP NGUYÊN VĂN 100% (VERBATIM)** toàn bộ mô tả của bà ấy (bao gồm Tên, Tuổi, Tóc, Mắt, Mặt, VÀ CẢ gợi ý trang phục) và DÁN vào prompt.

* **BƯỚC 2: ĐỌC VÀ SỬA (READ & ADAPT)**
    * NGAY SAU KHI DÁN, bạn phải đọc "Mô tả Kịch bản Chi tiết" của cảnh này.
    * BẮT BUỘC **SỬA LẠI** phần mô tả vừa dán để nó khớp với bối cảnh của cảnh.
    * Cụ thể: Sửa lại **Trang phục**, **Hành động**, và **Biểu cảm**.
    * *VÍ DỤ QUY TRÌNH:*
        * *Nguồn Chân Lý nói:* \`Haruka Sato: ... Trang phục thường ngày là áo chàm...\`
        * *Kịch bản Cảnh nói:* "Haruka đang mặc kimono trong phòng trà."
        * *AI thực hiện Bước 1 (Dán):* \`Haruka Sato, 70 tuổi, người Nhật, tóc bạc búi, mắt nâu hiền từ, nếp nhăn, trang phục thường ngày là áo chàm...\`
        * *AI thực hiện Bước 2 (Sửa):* \`Haruka Sato, 70 tuổi, người Nhật, tóc bạc búi, mắt nâu hiền từ, nếp nhăn, <xóa áo chàm> mặc một bộ kimono samue màu nâu nhạt, đang ngồi thanh thản...\`

* **CẢNH BÁO TỐI CAO:** Lặp lại quy trình "DÁN VÀ SỬA" này cho **MỌI NHÂN VẬT** được định nghĩa có mặt trong cảnh. Nếu cảnh có 3 nhân vật, tôi muốn thấy 3 MÔ TẢ ĐẦY ĐỦ ĐÃ ĐƯỢC SỬA. Nếu bạn chỉ viết tên ('Haruka Sato'), bạn đã THẤT BẠI.
...
1.  **Bảng Phân cảnh:** Trình bày dưới dạng bảng Markdown. BẮT ĐẦU VỚI DÒNG "### Bảng Phân cảnh". Bảng phải có 5 cột với tiêu đề chính xác như sau: "STT/Phân cảnh", "Thời gian (8 giây)", "Mô tả Kịch bản Chi tiết", "Prompt Tạo Ảnh (Whisk AI)", "Prompt Tạo Chuyện động (Veo 3.1)".
    ...
    * **Prompt Tạo Chuyển động (Veo 3.1):** Prompt riêng biệt bằng **TIẾNG ANH**, phải là một chuỗi JSON hợp lệ.
        * Nếu có nhân vật, BẮT BUỘC phải nhúng mô tả (đã sao chép và sửa) vào key \`character_definitions\` trong JSON.
`;
...
// =========================================================================
// === SỬA LỖI: CẬP NHẬT HÀM `generateScript` (LOGIC "MỘT TRONG HAI" v8.0) ===
// =========================================================================
export const generateScript = async (
  ...
  try {
    ...
    if (characterRefsList.length > 0) {
      ...
      finalCharacterDefinition = "Trống. Không có nhân vật nào được định nghĩa.";
    }
    // --- KẾT THÚC LOGIC MỚI ---
    ...
    systemPrompt = systemPrompt.replace('{{CHARACTER_DATA_GOES_HERE}}', finalCharacterDefinition); // <-- INJECTION
    ...
    // Đã xóa Nguồn Chân Lý khỏi đây, vì nó đã được "tiêm" vào systemPrompt
